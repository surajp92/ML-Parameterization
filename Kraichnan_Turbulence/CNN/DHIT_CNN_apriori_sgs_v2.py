...
class DHIT:
    def __init__(self,nx,ny,nxf,nyf,re,freq,n_snapshots,n_snapshots_train,n_snapshots_test,
        ...
        klist_test = [1,4]
        self.kruns = kruns
        self.klist_train = klist_train
        self.klist_test = klist_test
        
        self.wc = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.sc = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.kwc = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.ksc = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.pi = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.wcx = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.wcy = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.wcxx = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.wcyy = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.wcxy = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.scx = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.scy = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.scxx = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.scyy = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        self.scxy = np.zeros(shape=(kruns*self.n_snapshots, self.nx+1, self.ny+1), dtype='double')
        
        
        self.wc_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.sc_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.kwc_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.ksc_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.pi_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.wcx_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.wcy_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.wcxx_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.wcyy_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.wcxy_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.scx_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.scy_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.scxx_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.scyy_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        self.scxy_test = np.zeros(shape=(kruns, self.nx+1, self.ny+1), dtype='double')
        
        sn_count = 0
        for k in klist_train:
            folder = './run_'+str(k)+'/results/data_'+str(int(self.re))+'_fdns_'+str(self.nxf)+'_'+str(self.nx)
            
            for m in range(1,self.n_snapshots+1):    
                
                file_input = folder+'/results_' + str(int(m))+'.npz'
                data_input = np.load(file_input)
                self.wc[sn_count,:,:] = data_input['wc']
                self.sc[sn_count,:,:] = data_input['sc']
                self.kwc[sn_count,:,:] = data_input['kw']
                self.ksc[sn_count,:,:] = data_input['ks']
                self.pi[sn_count,:,:] = data_input['pi']
                self.wcx[sn_count,:,:] = data_input['wcx']
                self.wcy[sn_count,:,:] = data_input['wcy']
                self.wcxx[sn_count,:,:] = data_input['wcxx']
                self.wcyy[sn_count,:,:] = data_input['wcyy']
                self.wcxy[sn_count,:,:] = data_input['wcxy']
                self.scx[sn_count,:,:] = data_input['scx']
                self.scy[sn_count,:,:] = data_input['scy']
                self.scxx[sn_count,:,:] = data_input['scxx']
                self.scyy[sn_count,:,:] = data_input['scyy']
                self.scxy[sn_count,:,:] = data_input['scxy']
        ...
        for k in klist_test:
            ...
            file_input = folder+'/results_' + str(int(self.n_snapshots_test))+'.npz'
            data_input = np.load(file_input)
            self.wc_test[sn_count,:,:] = data_input['wc']
            self.sc_test[sn_count,:,:] = data_input['sc']
            self.kwc_test[sn_count,:,:] = data_input['kw']
            self.ksc_test[sn_count,:,:] = data_input['ks']
            self.pi_test[sn_count,:,:] = data_input['pi']
            self.wcx_test[sn_count,:,:] = data_input['wcx']
            self.wcy_test[sn_count,:,:] = data_input['wcy']
            self.wcxx_test[sn_count,:,:] = data_input['wcxx']
            self.wcyy_test[sn_count,:,:] = data_input['wcyy']
            self.wcxy_test[sn_count,:,:] = data_input['wcxy']
            self.scx_test[sn_count,:,:] = data_input['scx']
            self.scy_test[sn_count,:,:] = data_input['scy']
            self.scxx_test[sn_count,:,:] = data_input['scxx']
            self.scyy_test[sn_count,:,:] = data_input['scyy']
            self.scxy_test[sn_count,:,:] = data_input['scxy']
    ...
    def scale_data(self):
        ...
        self.wc_test = (2.0*self.wc_test - (np.max(self.wc) + np.min(self.wc)))/(np.max(self.wc) - np.min(self.wc))
        self.sc_test = (2.0*self.sc_test - (np.max(self.sc) + np.min(self.sc)))/(np.max(self.sc) - np.min(self.sc))
        self.kwc_test = (2.0*self.kwc_test - (np.max(self.kwc) + np.min(self.kwc)))/(np.max(self.kwc) - np.min(self.kwc))
        self.ksc_test = (2.0*self.ksc_test - (np.max(self.ksc) + np.min(self.ksc)))/(np.max(self.ksc) - np.min(self.ksc))
        
        self.wcx_test = (2.0*self.wcx_test - (np.max(self.wcx) + np.min(self.wcx)))/(np.max(self.wcx) - np.min(self.wcx))
        self.wcy_test = (2.0*self.wcy_test - (np.max(self.wcy) + np.min(self.wcy)))/(np.max(self.wcy) - np.min(self.wcy))
        self.wcxx_test = (2.0*self.wcxx_test - (np.max(self.wcxx) + np.min(self.wcxx)))/(np.max(self.wcxx) - np.min(self.wcxx))
        self.wcyy_test = (2.0*self.wcyy_test - (np.max(self.wcyy) + np.min(self.wcyy)))/(np.max(self.wcyy) - np.min(self.wcyy))
        self.wcxy_test = (2.0*self.wcxy_test - (np.max(self.wcxy) + np.min(self.wcxy)))/(np.max(self.wcxy) - np.min(self.wcxy))
        
        self.scx_test = (2.0*self.scx_test - (np.max(self.scx) + np.min(self.scx)))/(np.max(self.scx) - np.min(self.scx))
        self.scy_test = (2.0*self.scy_test - (np.max(self.scy) + np.min(self.scy)))/(np.max(self.scy) - np.min(self.scy))
        self.scxx_test = (2.0*self.scxx_test - (np.max(self.scxx) + np.min(self.scxx)))/(np.max(self.scxx) - np.min(self.scxx))
        self.scyy_test = (2.0*self.scyy_test - (np.max(self.scyy) + np.min(self.scyy)))/(np.max(self.scyy) - np.min(self.scyy))
        self.scxy_test = (2.0*self.scxy_test - (np.max(self.scxy) + np.min(self.scxy)))/(np.max(self.scxy) - np.min(self.scxy))
        
        self.pi_test = (2.0*self.pi_test - (np.max(self.pi) + np.min(self.pi)))/(np.max(self.pi) - np.min(self.pi))
    ...
    def gen_train_data(self):
        ...
        if self.ifeatures == 1:
            x_train  = np.zeros(shape=(self.kruns*self.n_snapshots_train, self.nx+1, self.ny+1, 2), dtype='double')
        elif self.ifeatures == 2:
            x_train  = np.zeros(shape=(self.kruns*self.n_snapshots_train, self.nx+1, self.ny+1, 4), dtype='double')
        elif self.ifeatures == 3:
            x_train  = np.zeros(shape=(self.kruns*self.n_snapshots_train, self.nx+1, self.ny+1, 12), dtype='double')
            
        if self.ilabel == 1:
            y_train = np.zeros(shape=(self.kruns*self.n_snapshots_train, self.nx+1, self.ny+1, 1), dtype='double')
        elif self.ilabel == 2:
            y_train = np.zeros(shape=(self.kruns*self.n_snapshots_train, self.nx+1, self.ny+1, 3), dtype='double')
...
#        for m in range(1,self.n_snapshots_train+1):
        ...
        if self.ifeatures == 3:
            ...
            x_train[:,:,:,8] = self.scy
            x_train[:,:,:,9] = self.scxx
            x_train[:,:,:,10] = self.scyy
            x_train[:,:,:,11] = self.scxy
    ...
    def gen_test_data(self):
        ...
        if self.ifeatures == 1:
            x_test  = np.zeros(shape=(self.kruns, self.nx+1, self.ny+1, 2), dtype='double')
        elif self.ifeatures == 2:
            x_test  = np.zeros(shape=(self.kruns, self.nx+1, self.ny+1, 4), dtype='double')
        elif self.ifeatures == 3:
            x_test  = np.zeros(shape=(self.kruns, self.nx+1, self.ny+1, 12), dtype='double')
            
        if self.ilabel == 1:
            y_test = np.zeros(shape=(self.kruns, self.nx+1, self.ny+1, 1), dtype='double')
        elif self.ilabel == 2:
            y_test = np.zeros(shape=(self.kruns, self.nx+1, self.ny+1, 3), dtype='double')
            
        m = self.n_snapshots_test

        if self.ifeatures == 1:
            x_test[:,:,:,0] = self.wc_test
            x_test[:,:,:,1] = self.sc_test
        if self.ifeatures == 2:
            x_test[:,:,:,0] = self.wc_test
            x_test[:,:,:,1] = self.sc_test
            x_test[:,:,:,2] = self.kwc_test
            x_test[:,:,:,3] = self.ksc_test
        if self.ifeatures == 3:
            x_test[:,:,:,0] = self.wc_test
            x_test[:,:,:,1] = self.sc_test
            x_test[:,:,:,2] = self.wcx_test
            x_test[:,:,:,3] = self.wcy_test
            x_test[:,:,:,4] = self.wcxx_test
            x_test[:,:,:,5] = self.wcyy_test
            x_test[:,:,:,6] = self.wcxy_test
            x_test[:,:,:,7] = self.scx_test
            x_test[:,:,:,8] = self.scy_test
            x_test[:,:,:,9] = self.scxx_test
            x_test[:,:,:,10] = self.scyy_test
            x_test[:,:,:,11] = self.scxy_test
        
        if self.ilabel == 1:
            y_test[:,:,:,0] = self.pi_test
        elif self.ilabel == 2:
            y_test[:,:,:,0] = self.t11
            y_test[:,:,:,1] = self.t12
            y_test[:,:,:,2] = self.t22            
...
class CNN:
    ...
    def CNN(self,x_train,y_train,nx,ny,nci,nco):
        ...
        x = Conv2D(num_filters , (size_nfilters, size_nfilters), activation='relu', padding='same')(input_img)
        x = Conv2D(num_filters , (size_nfilters, size_nfilters), activation='relu', padding='same')(x)
        x = Conv2D(num_filters , (size_nfilters, size_nfilters), activation='relu', padding='same')(x)
        x = Conv2D(num_filters , (size_nfilters, size_nfilters), activation='relu', padding='same')(x)
        x = Conv2D(num_filters , (size_nfilters, size_nfilters), activation='relu', padding='same')(x)
        x = Conv2D(num_filters , (size_nfilters, size_nfilters), activation='relu', padding='same')(x)
        decoded = Conv2D(nco, (size_nfilters, size_nfilters), activation='linear', padding='same')(x)
...
#        decoded = Conv2D(nco, (3, 3), activation='linear', padding='same')(x)
    ...
    def CNN_train(self,epochs,batch_size):
        ...
        X_train, X_valid, Y_train, Y_valid = train_test_split(self.x_train,self.y_train, test_size=0.2, random_state=1)
...
#x_train_sc = x_train_sc[:,:-1,:-1,:]
#y_train_sc = y_train_sc[:,:-1,:-1,:]
#x_test_sc = x_test_sc[:,:-1,:-1,:]
#y_test_sc = y_test_sc[:,:-1,:-1,:]
...
#%%
model.CNN_save('./nn_history/CNN_model_1_'+ str(re) + '_' + str(nxf) + '_' + str(nx) + '_' + str(ifeatures)+'.hd5')
np.save('./nn_history/scaling.npy',max_min)
...
cs = axs[1].contourf(y_pred[0,:,:,0].T, 120, cmap = 'jet', interpolation='bilinear')
